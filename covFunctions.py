...
def changePoint(log_steepness,log_center):
    def f(x1,x2):
        steepness = np.exp(log_steepness)
        center =  np.exp(log_center)
        sumx1 = np.array([np.sum(x1-center, axis=1)]).T
        x1= sumx1*steepness
        sumx2 = np.array([np.sum(x2-center, axis=1)]).T
        x2= sumx2*steepness
        sx = 1./(1+np.exp(-x1[0]))
        sz = 1./(1+np.exp(-x2[0])) # ToDo: why do I need to index here???
        A = np.dot(sx,sz.T)
        return A
    return f
...
def makeCP(log_steepness,log_center):
   return VentureFunction(changePoint(log_steepness,log_center), sp_type=covfunctionType,derivatives={},name="CP",parameter=[log_steepness,log_center])
